...
#plot(park.project, col="green", add=TRUE)
#ext( getSpPPolygonsLabptSlots(syr), labels=syr$NAME, cex=0.4 )
...
#load parks data
parks_census <- readShapePoly( fn = "SyracuseParks051414_UTM" , proj4string = CRS( "+proj=longlat +datum=WGS84" ) )

#subset roads within syracuse area
inter_clipped <- gIntersection(syr.shape, parks_census, byid = TRUE, drop_lower_td = TRUE)



par( mar=c(0,0,0,0) )
plot(syr.shape, border="gray", col="white")
text( getSpPPolygonsLabptSlots(syr), labels=syr$NAME, cex=0.4 )
plot(parks_census,add=T)

par( mar=c(1,1,1,1) )
plot(parks_census, border="green", col="darkgreen")
plot(syr.shape, border="gray", add=T)
text( getSpPPolygonsLabptSlots(syr), labels=syr$NAME, cex=0.4 )
...
ggplot(data=acre.by.neighborhood,aes(x=acre.by.neighborhood$Nhood, y=acre.by.neighborhood$ACRES))+
       labs(x = "Neighborhoods" ,y="Acres of Park", title="Acres of parks by Neighborhood")+geom_bar(colour = "blue",stat = "identity")+
