...
Subset yelp data to restaurants and bars that are within the city of Syracuse.

```{r, warning=F, message=F }
yelp.syr <- gIntersection(yelp, syr)
```
...
```{r, warning=F, message=F }
temp <- aggregate(dat$RATING[dat$RATING > 0], list(dat$FIPS[dat$RATING > 0]), FUN="mean")
tract.order <- match(yelp.data$TRACT, temp$Group.1)
yelp.data$RATING <- temp$x[tract.order]
yelp.data$RATING[is.na(yelp.data$RATING)] <- 0
```
...
```{r, warning=F, message=F }
color.order <- color.vector[yelp.data$TRACT]

plot(syr, border="white", col=color.order, main="Average Rating by Census Tract")
...
```{r, warning=F, message=F }
get.legend <- function(agg.dat,slices){
  dat.max <- max(agg.dat)
  dat.min <- min(agg.dat)
